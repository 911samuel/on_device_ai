...
Date of run: 2026-08-18. Targets: Android emulator (Pixel 8, arm64-v8a) and iOS Simulator (iPhone 17,
iOS 26.2).
...
| 6 | In-app socket self-test | Both | Implemented; meaningful on Android release only | Weak by itself |
| 7 | UI-driven inference in a **release** build | Android | **NOT COMPLETED** — emulator System UI was ANR-ing | — |
...
development. So this check is only meaningful for release builds, and the app's own UI says so rather than
letting a debug run imply more than it should.
...
package on disk. The only `dart:io` network call anywhere in `lib/` is the deliberate self-test in
`lib/application/network_self_test.dart`; the app declares no HTTP client and depends on no networking
package. Grep-able claim:

```bash
grep -rn "http\|Socket\|HttpClient" lib/ --include=*.dart
```

returns only `network_self_test.dart`.

## 6. The in-app self-test, and its limits

The app has a **Run network self-test** button that asks the OS for a real outbound TCP socket to
`1.1.1.1:443` and reports the outcome. Its value depends entirely on the platform:

* **Android release** — the socket is refused; inference keeps working. That is a capability proof.
* **Android debug/profile** — the socket succeeds, because Flutter's tooling manifest added INTERNET. The
  UI shows an explicit warning in this mode so the result cannot be misread.
* **iOS** — there is no install-time network permission on iOS, so a successful probe says nothing at all
  about whether inference used the network. Local execution on iOS rests on checks 4 and 5 plus the
  architecture: the model bytes come from the bundle and the runtime is a linked native library.
